...
tsp_loop_return dsp_loop( tsp_loop_param data );
...
static inline uint8_t mp4_parse_box_AudioSampleEntry( int8_t* err, mp4parse_t* mp4parse, uint32_t idx )
{
    ...
    {
        LOG_MSG( mp4p->log_lvl, LOG_LOW, "[MP4PARSE][%s] AudioSampleEntry: V1 t.b.d\n", mp4p->label );
...
static inline void mp4_parse_obj_DecoderSpecificInfo( int8_t* err, mp4parse_t* mp4parse, uint32_t idx )
{
    ...
    // mp4p->mp4_audio_specific_info = (mp4p->data_raw+i);
...
tsp_loop_return dsp_loop( tsp_loop_param data )
{
    ...
    if ( mp4p == NULL )
    {
        LOG_ERR( 0, LOG_ALWAYS, "[MP4PARSE][-][ERROR] dsp_loop, no mp4parse base component\n" );
        myerr = MP4PARSE_ERR_UNKNOWN;
        TSP_THREAD_EXIT( );
    ...
    else
    {
        ...
        {
            LOG_ERR( 0, LOG_ALWAYS, "[MP4PARSE][-][ERROR] dsp_loop, not a valid mp4parse component\n" );
            myerr = MP4PARSE_ERR_UNKNOWN;
            TSP_THREAD_EXIT( );
            TSP_THREAD_RETURN_NULL( );
        }
    ...
    {
        myerr = MP4PARSE_ERR_UNKNOWN;
        LOG_ERR( 0, LOG_ALWAYS, "[MP4PARSE][%s][ERROR] dsp_loop, mp4parse NOT in ready state\n", mp4p->label );
    }
    ...
    {
        myerr = MP4PARSE_ERR_UNKNOWN;
        LOG_ERR( 0, LOG_ALWAYS, "[MP4PARSE][%s][ERROR] dsp_loop, no ctrl\n", mp4p->label );
    }
    ...
    if ( NO_ERR == myerr )
    {
        ...
        *ctrl = ( *ctrl ) | MP4PARSE_CTRL_DSP_STATUS;
        ...
        {
            LOG_MSG( mp4p->log_lvl, LOG_LOW, "[MP4PARSE][%s] dsp_loop (STARTPING) \n", mp4p->label );
            do
            {
                /** DSP thread
                ...
                if ( TRUE == fdk_f )
                {
                    ...
                    LOG_MSG( mp4p->log_lvl, LOG_LOW, "[MP4PARSE][%s] dsp_loop: initialize FDK (%p %" PRIu32 ") \n",
                             mp4p->label,
                    ...
                    {
                        LOG_ERR( 0, LOG_ALWAYS, "[MP4PARSE][%s] [ERROR] dsp_loop: unable to initialize FDK with the ASC provided.\n",
                                 mp4p->label );
                    ...
                    else
                    {
                        ...
                        if ( mp4p->fdk_err != AAC_DEC_OK )
                        {
                            LOG_ERR( 0, LOG_ALWAYS, "[MP4PARSE][%s] [ERROR] dsp_loop: unable to set FDK optional paramters.\n",
                                     mp4p->label );
                ...
                if ( TRUE == fseek_f )
                {
                         ...
                         ( chunk_bitstream_offset + sample_chunk_offset + sample_size ) )
                    {
                        ...
                        {
                            LOG_MSG( mp4p->log_lvl, LOG_LOW, "[MP4PARSE][%s] dsp_loop: FSEEK-> %" PRIu32 " of %" PRIu32 " "
        ...
        {
            myerr = MP4PARSE_ERR_UNKNOWN;
            LOG_ERR( 0, LOG_ALWAYS, "[MP4PARSE][%s][ERROR] dsp_loop could not update the ctrl register.\n", mp4p->label );
        }
    }
    /* *** */
    ...
    *ctrl = ( *ctrl ) & ( ~( MP4PARSE_CTRL_DSP_STATUS ) );
    ...
    LOG_MSG( 0, LOG_ALWAYS, "[MP4PARSE][%s] dsp_loop quit. (%" PRIu8 ") \n", mp4p->label, myerr );
