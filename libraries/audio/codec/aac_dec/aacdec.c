                 ...
                 uint8_t            downmix_f )
{
    ...
    if ( NO_ERR == myerr )
    {
        ...
        {
            LOG_ERR( 0, LOG_ALWAYS, "[MP4PARSE][-] [ERROR] aadec_new, output buf create FAIL\n" );
            myerr = -1;
        }
        else
        {
                                ...
                                uint32_t buf_len )
{
    ...
    if ( NO_ERR == myerr )
    {
        LOG_MSG( 0, LOG_HIGH, "[AACDEC][%s] aacdec_input_push_buf, size=%" PRIu32 "\n", adec->label, buf_len );

        push_byte_cnt = mp4parse_input_push_buf( &myerr, adec->mp4parse, buf, buf_len );
    }
