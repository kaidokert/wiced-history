                                 ...
                                 uint32_t*          buf_id )
{
    ...
    if ( buf_id == NULL )
    {
        if ( dec->pcm_first_buffer )
        {
            ...
            if ( audio_client_render_configure( dec->audio_client->audio, &dec->audio_config ) != WICED_SUCCESS )
            {
            ...
            if(dec->audio_client->effect_mode != AUDIO_CLIENT_EFFECT_MODE_NONE)
            {
                audio_client_render_command(dec->audio_client->audio, AUDIO_CLIENT_RENDER_CMD_SET_EFFECT, (void*)dec->audio_client->effect_mode);
            }
        }
        ...
        {
            result = audio_client_render_buffer_get( dec->audio_client->audio, &audio_buf );
            ...
            {
                result = audio_client_render_buffer_push( dec->audio_client->audio, audio_buf );
                if ( result == WICED_SUCCESS )
                {
                ...
                {
                    wiced_log_msg( WICED_LOG_ERR, "audio_client_aac: Error pushing audio buffer to the audio_render\n" );
                    return WICED_ERROR;
                }
            }
        }
    ...
    if ( buf_id != NULL )
    {
        if ( *buf_id == 0 )
        {
             ...
             */
            audio_client_render_buffer_push( dec->audio_client->audio, NULL );
        }
    }
...
static wiced_result_t process_aac_data( audio_client_t* client )
{
    ...
    /*
     * push to the aac_dec component
     */
    int8_t         rv;

    aacdec_input_push_buf( &rv, dec->aac_dec_handle, (char*) dbuf->buf, (uint32_t) dbuf->buflen );
...
wiced_result_t audio_client_aac_decoder_start( audio_client_t* client )
{
    ...
    if ( NULL != dec )
    {
        aacdec_new( NULL,
                    ...
                    dec,
                    0,                        /* LOG_NONE */
                    AACDEC_STREAM_M4A,        /* MP4 STREAM TYPE  */
                    NULL,                     /* PCM_TARGET_FMT   */
