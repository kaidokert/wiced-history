...
int connmgr_init(void) {
	...
	if (buf == NULL) {
	    Log_error("connmgr", "ERROR: malloc() failed in %s()", __FUNCTION__);
		return -1;
	}
	...
	for (entry = supported_types; entry; entry = entry->next) {
		...
		if (buf == NULL) {
		    Log_error("connmgr", "ERROR: realloc() failed in %s()", __FUNCTION__);
			return -1;
		}
		p = buf;
		p += offset;
...
int connmgr_deinit(void)
{
    ithread_mutex_destroy(&connmgr_mutex);
    return 0;
}
...
struct service *upnp_connmgr_get_service(void) {
    ...
    connmgr_service_.variable_container = NULL;
    ...
    ithread_mutex_init(&connmgr_mutex, NULL);
