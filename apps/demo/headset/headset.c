 ...
 * Headset Application
 *
 * This application demonstrates a wireless Headset which works with audio player.
 *
...
static void headset_worker_thread( uint32_t args )
{
    ...
    wiced_log_msg(WICED_LOG_NOTICE, "Headset Worker thread started...\n");
    ...
    while( WICED_TRUE )
    {
        ...
        {
            wiced_log_msg(WICED_LOG_ERR, "Error receiving event from app-queue\n");
            continue;
        }
...
wiced_result_t app_set_service_state(service_type_t service_type, service_state_t state)
{
    ...
    {
        wiced_log_msg(WICED_LOG_ERR, "%s(): new service type is NONE. Can't proceed\n", __func__);
        return WICED_ERROR;
    }
    ...
    if (service == NULL)
    {
        wiced_log_msg(WICED_LOG_ERR, "%s(): new service is NULL. Can't proceed\n", __func__);
        wiced_rtos_unlock_mutex( &g_headset_context->lock );
        return WICED_ERROR;
    }
...
service_state_t app_get_service_state(service_type_t service_type)
{
    ...
    if(service_type == SERVICE_NONE)
    {
        wiced_log_msg(WICED_LOG_ERR, "%s(): service type is NONE. Can't proceed\n", __func__);
        return WICED_ERROR;
    }
    ...
    if (service == NULL)
    {
        wiced_log_msg(WICED_LOG_ERR,"%s(): service is NULL. Can't proceed\n", __func__);
        return WICED_ERROR;
    }
...
wiced_result_t app_set_current_service(service_type_t service_type)
{
    ...
    if( current != NULL )
    {
        ...
        {
            wiced_log_msg(WICED_LOG_ERR, "New service[%d] lower priority than current [%d]..Aborting !!\n", new_service->priority, current->priority);
            goto SET_UNLOCK_AND_END;
        }
    }
...
static wiced_result_t headset_init(headset_context_t** _headset_context)
{
    ...
    HEADSET_CHECK_RESULT(wiced_log_init(WICED_LOG_NOTICE, headset_log_output_handler, NULL));
    ...
    {
        wiced_log_msg(WICED_LOG_ERR, "Unable to allocation headset context\n");
        return WICED_ERROR;
    }
    ...
    /* Init gmediarender */
    HEADSET_CHECK_RESULT(headset_gmrender_init(&headset_context->gmrender_context,
    ...
    platform_tick_stop();
    platform_cpu_clock_init(HEADSET_DCT_POWERSAVE_PARAMS(headset_context).mcu_clock_freq);
    platform_tick_start();
...
#endif
    /* Gmediarender */
    HEADSET_CHECK_RESULT(headset_gmrender_deinit(headset_context->gmrender_context));
    /* WLAN service */
...
void application_start(void)
{
    ...
    HEADSET_CHECK_RESULT(headset_wlan_application_start(&headset_context->wlan_interface, headset_context->gmrender_context));
    ...
    wiced_log_msg(WICED_LOG_ERR, "Headset demo is started ...\n");
    return;
_exit:
    wiced_log_msg(WICED_LOG_ERR, "Something wrong !!! System will be reboot\n");
