...
static wiced_result_t service_volume_up(void)
{
    ...
    {
        wiced_log_msg(WICED_LOG_ERR, "%s(): Invalid Service\n", __func__);
        return result;
    }
...
static wiced_result_t service_wlan_connect(void)
{
    ...
    {
        wiced_log_msg(WICED_LOG_ERR, "%s(): Failed to get service entry\n", __func__);
        return WICED_ERROR;
    }
    ...
    if (app_get_service_state(SERVICE_WLAN) != SERVICE_DISABLED)
    {
        ...
        result = service->button_handler(ACTION_WLAN_DEINIT);
    }
    else
    {
        /* WLAN is default down for saving more power
         * So we need to init here when we want to start WLAN headset */
        result = service->button_handler(ACTION_WLAN_INIT);
...
static wiced_result_t service_EQ_setting(void)
{
    ...
    wiced_log_msg(WICED_LOG_NOTICE, "service_EQ_setting\n");
...
static void app_button_event_handler( const button_manager_button_t* button, button_manager_event_t event, button_manager_button_state_t state )
{
    ...
    switch ( button_event_id )
    {
        ...
        default: wiced_log_msg(WICED_LOG_DEBUG0, "Button unhandled button_event_id: %x\n", (unsigned int)button_event_id); return;
    }
