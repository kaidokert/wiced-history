 ...
 *                      Macros
 ******************************************************/
#define HEADSET_TRACE_IN             wiced_log_msg(WICED_LOG_DEBUG2, "%s In (%d)\n", __func__, __LINE__);
#define HEADSET_TRACE_OUT            wiced_log_msg(WICED_LOG_DEBUG2, "%s Out (%d)\n", __func__, __LINE__);
#define HEADSET_CHECK_RESULT(expr)   result = (wiced_result_t)(expr);\
                                     if (result != WICED_SUCCESS)\
                                     {wiced_log_msg(WICED_LOG_ERR, "Error@%s(%d)\n", __func__, __LINE__);\
                                      goto _exit;\
                                     }\
...
typedef struct
{
    ...
    /* Gmediarender */
    headset_gmrender_context_t*    gmrender_context;
} headset_context_t;
